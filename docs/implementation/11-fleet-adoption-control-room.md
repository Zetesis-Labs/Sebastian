...
- Born adopted (RF-03): a unit provisioned from the embedded installer holds
  the organization secret; its control room does not hold its secret yet
  (`cr_bound` in NVS ≠ origin of the token URL). Before its first poll
  `control.zig` enrols: `GET /v1/devices/{id}/enroll` → nonce,
  `POST …/enroll {nonce, mac, deviceSecret}` with `mac = HMAC-SHA256(orgSecret,
  nonce + "." + id)` (the adoption construction). The server marks the unit
  adopted like a network adoption would; the nonce is single-use, 60 s. A 401
  on the poll clears `cr_bound`, so the unit re-enrols.
- The device secret is born with the unit (`sebastian_ensure_device_secret`,
...
| **Device secret** | one speaker | born with the unit (generated at first boot, kept across adoptions); handed to the adopting control room in the `ok` reply / the enrolment; shown in the dashboard at adoption (copy/QR) and readable over USB | `/v1/sessions` auth, and handing a single unit to a third party; rotation from the ficha is optional |
