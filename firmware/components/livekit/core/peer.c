...
peer_err_t peer_create(peer_handle_t *handle, peer_options_t *options)
{
    ...
    esp_peer_cfg_t peer_cfg = {
        ...
        // LOCAL PATCH (esp-webrtc-solution#186): only the PUBLISHER peer gets a
        // data channel; the subscriber's SCTP never associates and only produces
        // the INIT storm. Hardware-established peer↔role map (2026-07-08, tested
        // against BOTH LiveKit Cloud and self-hosted v1.13.3):
        //   - PUBLISHER peer → SCTP role 0 (passive): associates, opens DCEP
        //     _reliable/_lossy, and carries ALL working data both ways (pre-roll,
        //     barge_in out; agent_state in).
        //   - SUBSCRIBER peer → SCTP role 1 (active): its INIT is never answered
        //     (DTLS for data never completes server-side); esp_peer retransmits
        //     INIT with no backoff at ~9/s for the entire session = the storm.
        // Disabling the subscriber DC was verified the RIGHT way around by the
        // inverse experiment: disabling the publisher DC instead killed pre-roll
        // → no mic handoff → level=0 silence-timeout sessions ("didn't hear me").
        .enable_data_channel = options->role == PEER_ROLE_PUBLISHER,
        .manual_ch_create = true,
        // NEGATIVE RESULT (2026-07-08, esp-webrtc-solution#186): setting this to
        // true does NOT stop the SCTP INIT storm — tested on hardware against the
        // self-hosted SFU: 398 INITs in 43s (~9/s), identical to the false
        // baseline (246 in 23s). The flood is the SCTP association's own
        // no-backoff INIT retransmission while the publisher PC's DTLS never
        // completes (SFU logs `dtls timeout` on the PUBLISHER transport); this
        // flag only gates full-peer reconnect-after-failure, which never fires
        // here. Left at false (default): flipping it doesn't help the storm and
        // would hurt legitimate reconnects.
