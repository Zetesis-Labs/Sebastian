...
pub extern fn sebastian_session_create(base_url: [*:0]const u8, device_id: [*:0]const u8, secret: [*:0]const u8, json_body: ?[*:0]const u8, url_out: [*]u8, url_size: usize, token_out: [*]u8, token_size: usize) c_int;
pub extern fn sebastian_request_meeting(base_url: [*:0]const u8, device_id: [*:0]const u8, secret: [*:0]const u8, id_out: [*]u8, id_size: usize) c_int;
pub extern fn sebastian_report_meeting(base_url: [*:0]const u8, device_id: [*:0]const u8, secret: [*:0]const u8, meeting_id: [*:0]const u8, state: [*:0]const u8, reason: [*:0]const u8) c_int;
...
pub extern fn sebastian_http_get_auth2(url: [*:0]const u8, device_id: ?[*:0]const u8, secret: ?[*:0]const u8, header1: ?[*:0]const u8, hdr1_out: ?[*]u8, hdr1_size: usize, header2: ?[*:0]const u8, hdr2_out: ?[*]u8, hdr2_size: usize, out: [*]u8, out_size: usize, status: ?*c_int) c_int;
pub extern fn sebastian_meeting_push(cmd: [*:0]const u8, id: [*:0]const u8, origin: u8) void;
pub extern fn sebastian_meeting_take(cmd: [*]u8, cmd_size: usize, id: [*]u8, id_size: usize, origin: ?*u8) bool;
pub extern fn sebastian_meeting_set_active(active: bool) void;
pub extern fn sebastian_meeting_is_active() bool;
pub extern fn sebastian_meeting_set_recordable(recordable: bool) void;
pub extern fn sebastian_meeting_is_recordable() bool;
...
pub extern fn sebastian_adopt_consent_grant() void;
...
// The layouts above are transcribed by hand from C headers: an IDF or
// LiveKit/esp_capture/esp_codec_dev/av_render upgrade that changes a field,
// its order or its alignment breaks them SILENTLY at runtime. This block is
// the Zig half of the contract; main/abi_check.c is the C half, checking the
// same literals against the real headers. A failed assert here means the Zig
// declaration drifted; a failed _Static_assert there means the headers did.
// Keep both in sync.
//
// Layouts assume the ESP32-S3 ILP32 ABI (pointers/usize 4 bytes, enums 4,
// u64 8-aligned). Host builds never import this file, so nothing here is
// evaluated off-target. Literals verified against IDF v5.4 and the vendored
// components in managed_components/ (2026-07).
comptime {
    // --- IDF esp_driver_i2c ---
    std.debug.assert(@sizeOf(i2c_master_bus_config_t) == 32);
    std.debug.assert(@offsetOf(i2c_master_bus_config_t, "i2c_port") == 0);
    std.debug.assert(@offsetOf(i2c_master_bus_config_t, "sda_io_num") == 4);
    std.debug.assert(@offsetOf(i2c_master_bus_config_t, "scl_io_num") == 8);
    std.debug.assert(@offsetOf(i2c_master_bus_config_t, "clk_source") == 12);
    std.debug.assert(@offsetOf(i2c_master_bus_config_t, "glitch_ignore_cnt") == 16);
    std.debug.assert(@offsetOf(i2c_master_bus_config_t, "intr_priority") == 20);
    std.debug.assert(@offsetOf(i2c_master_bus_config_t, "trans_queue_depth") == 24);
    std.debug.assert(@offsetOf(i2c_master_bus_config_t, "flags") == 28);
    std.debug.assert(@sizeOf(i2c_device_config_t) == 20);
    std.debug.assert(@offsetOf(i2c_device_config_t, "dev_addr_length") == 0);
    std.debug.assert(@offsetOf(i2c_device_config_t, "device_address") == 4);
    std.debug.assert(@offsetOf(i2c_device_config_t, "scl_speed_hz") == 8);
    std.debug.assert(@offsetOf(i2c_device_config_t, "scl_wait_us") == 12);
    std.debug.assert(@offsetOf(i2c_device_config_t, "flags") == 16);

    // --- IDF esp_driver_i2s ---
    std.debug.assert(@sizeOf(i2s_chan_config_t) == 24);
    std.debug.assert(@offsetOf(i2s_chan_config_t, "id") == 0);
    std.debug.assert(@offsetOf(i2s_chan_config_t, "role") == 4);
    std.debug.assert(@offsetOf(i2s_chan_config_t, "dma_desc_num") == 8);
    std.debug.assert(@offsetOf(i2s_chan_config_t, "dma_frame_num") == 12);
    std.debug.assert(@offsetOf(i2s_chan_config_t, "auto_clear_after_cb") == 16);
    std.debug.assert(@offsetOf(i2s_chan_config_t, "auto_clear_before_cb") == 17);
    std.debug.assert(@offsetOf(i2s_chan_config_t, "allow_pd") == 18);
    std.debug.assert(@offsetOf(i2s_chan_config_t, "intr_priority") == 20);
    std.debug.assert(@sizeOf(i2s_std_clk_config_t) == 16);
    std.debug.assert(@offsetOf(i2s_std_clk_config_t, "sample_rate_hz") == 0);
    std.debug.assert(@offsetOf(i2s_std_clk_config_t, "clk_src") == 4);
    std.debug.assert(@offsetOf(i2s_std_clk_config_t, "ext_clk_freq_hz") == 8);
    std.debug.assert(@offsetOf(i2s_std_clk_config_t, "mclk_multiple") == 12);
    std.debug.assert(@sizeOf(i2s_std_slot_config_t) == 28);
    std.debug.assert(@offsetOf(i2s_std_slot_config_t, "data_bit_width") == 0);
    std.debug.assert(@offsetOf(i2s_std_slot_config_t, "slot_bit_width") == 4);
    std.debug.assert(@offsetOf(i2s_std_slot_config_t, "slot_mode") == 8);
    std.debug.assert(@offsetOf(i2s_std_slot_config_t, "slot_mask") == 12);
    std.debug.assert(@offsetOf(i2s_std_slot_config_t, "ws_width") == 16);
    std.debug.assert(@offsetOf(i2s_std_slot_config_t, "ws_pol") == 20);
    std.debug.assert(@offsetOf(i2s_std_slot_config_t, "bit_shift") == 21);
    std.debug.assert(@offsetOf(i2s_std_slot_config_t, "left_align") == 22);
    std.debug.assert(@offsetOf(i2s_std_slot_config_t, "big_endian") == 23);
    std.debug.assert(@offsetOf(i2s_std_slot_config_t, "bit_order_lsb") == 24);
    std.debug.assert(@sizeOf(i2s_std_gpio_config_t) == 24);
    std.debug.assert(@offsetOf(i2s_std_gpio_config_t, "mclk") == 0);
    std.debug.assert(@offsetOf(i2s_std_gpio_config_t, "bclk") == 4);
    std.debug.assert(@offsetOf(i2s_std_gpio_config_t, "ws") == 8);
    std.debug.assert(@offsetOf(i2s_std_gpio_config_t, "dout") == 12);
    std.debug.assert(@offsetOf(i2s_std_gpio_config_t, "din") == 16);
    std.debug.assert(@offsetOf(i2s_std_gpio_config_t, "invert_flags") == 20);
    std.debug.assert(@sizeOf(i2s_std_config_t) == 68);
    std.debug.assert(@offsetOf(i2s_std_config_t, "clk_cfg") == 0);
    std.debug.assert(@offsetOf(i2s_std_config_t, "slot_cfg") == 16);
    std.debug.assert(@offsetOf(i2s_std_config_t, "gpio_cfg") == 44);

    // --- esp_codec_dev ---
    std.debug.assert(@sizeOf(esp_codec_dev_sample_info_t) == 12);
    std.debug.assert(@offsetOf(esp_codec_dev_sample_info_t, "bits_per_sample") == 0);
    std.debug.assert(@offsetOf(esp_codec_dev_sample_info_t, "channel") == 1);
    std.debug.assert(@offsetOf(esp_codec_dev_sample_info_t, "channel_mask") == 2);
    std.debug.assert(@offsetOf(esp_codec_dev_sample_info_t, "sample_rate") == 4);
    std.debug.assert(@offsetOf(esp_codec_dev_sample_info_t, "mclk_multiple") == 8);
    std.debug.assert(@sizeOf(audio_codec_if_t) == 52);
    std.debug.assert(@offsetOf(audio_codec_if_t, "open") == 0);
    std.debug.assert(@offsetOf(audio_codec_if_t, "close") == 48);
    std.debug.assert(@sizeOf(audio_codec_i2s_cfg_t) == 12);
    std.debug.assert(@offsetOf(audio_codec_i2s_cfg_t, "port") == 0);
    std.debug.assert(@offsetOf(audio_codec_i2s_cfg_t, "rx_handle") == 4);
    std.debug.assert(@offsetOf(audio_codec_i2s_cfg_t, "tx_handle") == 8);
    std.debug.assert(@sizeOf(esp_codec_dev_cfg_t) == 12);
    std.debug.assert(@offsetOf(esp_codec_dev_cfg_t, "dev_type") == 0);
    std.debug.assert(@offsetOf(esp_codec_dev_cfg_t, "codec_if") == 4);
    std.debug.assert(@offsetOf(esp_codec_dev_cfg_t, "data_if") == 8);

    // --- av_render ---
    std.debug.assert(@sizeOf(i2s_render_cfg_t) == 16);
    std.debug.assert(@offsetOf(i2s_render_cfg_t, "play_handle") == 0);
    std.debug.assert(@offsetOf(i2s_render_cfg_t, "cb") == 4);
    std.debug.assert(@offsetOf(i2s_render_cfg_t, "fixed_clock") == 8);
    std.debug.assert(@offsetOf(i2s_render_cfg_t, "ctx") == 12);
    std.debug.assert(@sizeOf(av_render_cfg_t) == 40);
    std.debug.assert(@offsetOf(av_render_cfg_t, "audio_render") == 0);
    std.debug.assert(@offsetOf(av_render_cfg_t, "video_render") == 4);
    std.debug.assert(@offsetOf(av_render_cfg_t, "sync_mode") == 8);
    std.debug.assert(@offsetOf(av_render_cfg_t, "audio_raw_fifo_size") == 12);
    std.debug.assert(@offsetOf(av_render_cfg_t, "video_raw_fifo_size") == 16);
    std.debug.assert(@offsetOf(av_render_cfg_t, "audio_render_fifo_size") == 20);
    std.debug.assert(@offsetOf(av_render_cfg_t, "video_render_fifo_size") == 24);
    std.debug.assert(@offsetOf(av_render_cfg_t, "quit_when_eos") == 28);
    std.debug.assert(@offsetOf(av_render_cfg_t, "allow_drop_data") == 29);
    std.debug.assert(@offsetOf(av_render_cfg_t, "pause_render_only") == 30);
    std.debug.assert(@offsetOf(av_render_cfg_t, "pause_on_first_frame") == 31);
    std.debug.assert(@offsetOf(av_render_cfg_t, "ctx") == 32);
    std.debug.assert(@offsetOf(av_render_cfg_t, "video_cvt_in_render") == 36);
    std.debug.assert(@sizeOf(av_render_audio_frame_info_t) == 8);
    std.debug.assert(@offsetOf(av_render_audio_frame_info_t, "channel") == 0);
    std.debug.assert(@offsetOf(av_render_audio_frame_info_t, "bits_per_sample") == 1);
    std.debug.assert(@offsetOf(av_render_audio_frame_info_t, "sample_rate") == 4);

    // --- LiveKit SDK ---
    std.debug.assert(@sizeOf(livekit_video_encode_options_t) == 16);
    std.debug.assert(@offsetOf(livekit_video_encode_options_t, "codec") == 0);
    std.debug.assert(@offsetOf(livekit_video_encode_options_t, "width") == 4);
    std.debug.assert(@offsetOf(livekit_video_encode_options_t, "height") == 8);
    std.debug.assert(@offsetOf(livekit_video_encode_options_t, "fps") == 12);
    std.debug.assert(@sizeOf(livekit_audio_encode_options_t) == 12);
    std.debug.assert(@offsetOf(livekit_audio_encode_options_t, "codec") == 0);
    std.debug.assert(@offsetOf(livekit_audio_encode_options_t, "sample_rate") == 4);
    std.debug.assert(@offsetOf(livekit_audio_encode_options_t, "channel_count") == 8);
    std.debug.assert(@sizeOf(livekit_pub_options_t) == 36);
    std.debug.assert(@offsetOf(livekit_pub_options_t, "kind") == 0);
    std.debug.assert(@offsetOf(livekit_pub_options_t, "video_encode") == 4);
    std.debug.assert(@offsetOf(livekit_pub_options_t, "audio_encode") == 20);
    std.debug.assert(@offsetOf(livekit_pub_options_t, "capturer") == 32);
    std.debug.assert(@sizeOf(livekit_sub_options_t) == 8);
    std.debug.assert(@offsetOf(livekit_sub_options_t, "kind") == 0);
    std.debug.assert(@offsetOf(livekit_sub_options_t, "renderer") == 4);
    std.debug.assert(@sizeOf(livekit_participant_info_t) == 24);
    std.debug.assert(@offsetOf(livekit_participant_info_t, "sid") == 0);
    std.debug.assert(@offsetOf(livekit_participant_info_t, "identity") == 4);
    std.debug.assert(@offsetOf(livekit_participant_info_t, "name") == 8);
    std.debug.assert(@offsetOf(livekit_participant_info_t, "metadata") == 12);
    std.debug.assert(@offsetOf(livekit_participant_info_t, "kind") == 16);
    std.debug.assert(@offsetOf(livekit_participant_info_t, "state") == 20);
    // total_length also pins the 8-byte alignment of u64 on Xtensa.
    std.debug.assert(@sizeOf(livekit_data_stream_options_t) == 24);
    std.debug.assert(@offsetOf(livekit_data_stream_options_t, "topic") == 0);
    std.debug.assert(@offsetOf(livekit_data_stream_options_t, "is_text") == 4);
    std.debug.assert(@offsetOf(livekit_data_stream_options_t, "total_length") == 8);
    std.debug.assert(@offsetOf(livekit_data_stream_options_t, "has_total_length") == 16);
    std.debug.assert(@sizeOf(livekit_data_payload_t) == 8);
    std.debug.assert(@offsetOf(livekit_data_payload_t, "bytes") == 0);
    std.debug.assert(@offsetOf(livekit_data_payload_t, "size") == 4);
    std.debug.assert(@sizeOf(livekit_data_received_t) == 16);
    std.debug.assert(@offsetOf(livekit_data_received_t, "payload") == 0);
    std.debug.assert(@offsetOf(livekit_data_received_t, "topic") == 8);
    std.debug.assert(@offsetOf(livekit_data_received_t, "sender_identity") == 12);
    std.debug.assert(@sizeOf(livekit_room_options_t) == 68);
    std.debug.assert(@offsetOf(livekit_room_options_t, "publish") == 0);
    std.debug.assert(@offsetOf(livekit_room_options_t, "subscribe") == 36);
    std.debug.assert(@offsetOf(livekit_room_options_t, "on_state_changed") == 44);
    std.debug.assert(@offsetOf(livekit_room_options_t, "on_rpc_result") == 48);
    std.debug.assert(@offsetOf(livekit_room_options_t, "on_data_received") == 52);
    std.debug.assert(@offsetOf(livekit_room_options_t, "on_room_info") == 56);
    std.debug.assert(@offsetOf(livekit_room_options_t, "on_participant_info") == 60);
    std.debug.assert(@offsetOf(livekit_room_options_t, "ctx") == 64);
    std.debug.assert(@sizeOf(livekit_data_publish_options_t) == 20);
    std.debug.assert(@offsetOf(livekit_data_publish_options_t, "payload") == 0);
    std.debug.assert(@offsetOf(livekit_data_publish_options_t, "topic") == 4);
    std.debug.assert(@offsetOf(livekit_data_publish_options_t, "lossy") == 8);
    std.debug.assert(@offsetOf(livekit_data_publish_options_t, "destination_identities") == 12);
    std.debug.assert(@offsetOf(livekit_data_publish_options_t, "destination_identities_count") == 16);

    // --- esp_capture ---
    std.debug.assert(@sizeOf(esp_capture_audio_aec_src_cfg_t) == 12);
    std.debug.assert(@offsetOf(esp_capture_audio_aec_src_cfg_t, "mic_layout") == 0);
    std.debug.assert(@offsetOf(esp_capture_audio_aec_src_cfg_t, "record_handle") == 4);
    std.debug.assert(@offsetOf(esp_capture_audio_aec_src_cfg_t, "channel") == 8);
    std.debug.assert(@offsetOf(esp_capture_audio_aec_src_cfg_t, "channel_mask") == 9);
    std.debug.assert(@offsetOf(esp_capture_audio_aec_src_cfg_t, "data_on_vad") == 10);
    std.debug.assert(@sizeOf(esp_capture_cfg_t) == 12);
    std.debug.assert(@offsetOf(esp_capture_cfg_t, "sync_mode") == 0);
    std.debug.assert(@offsetOf(esp_capture_cfg_t, "audio_src") == 4);
    std.debug.assert(@offsetOf(esp_capture_cfg_t, "video_src") == 8);
    std.debug.assert(@sizeOf(esp_capture_audio_dev_src_cfg_t) == 4);
    std.debug.assert(@offsetOf(esp_capture_audio_dev_src_cfg_t, "record_handle") == 0);
    std.debug.assert(@sizeOf(esp_capture_audio_info_t) == 12);
    std.debug.assert(@offsetOf(esp_capture_audio_info_t, "format_id") == 0);
    std.debug.assert(@offsetOf(esp_capture_audio_info_t, "sample_rate") == 4);
    std.debug.assert(@offsetOf(esp_capture_audio_info_t, "channel") == 8);
    std.debug.assert(@offsetOf(esp_capture_audio_info_t, "bits_per_sample") == 9);
    std.debug.assert(@sizeOf(esp_capture_stream_frame_t) == 16);
    std.debug.assert(@offsetOf(esp_capture_stream_frame_t, "stream_type") == 0);
    std.debug.assert(@offsetOf(esp_capture_stream_frame_t, "pts") == 4);
    std.debug.assert(@offsetOf(esp_capture_stream_frame_t, "data") == 8);
    std.debug.assert(@offsetOf(esp_capture_stream_frame_t, "size") == 12);
