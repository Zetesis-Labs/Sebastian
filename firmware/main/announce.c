...
// room it is bound to, the result of the last contact and the config version.
...
void sebastian_announce_start(const char *id, const char *prof, const char *cr) {
    ...
    char last_err[48] = {0};
    const char *boot_err = sebastian_take_last_error(last_err, sizeof(last_err)) ? last_err : "";
    char cfg[24] = {0};
    ...
    started = true;
    ESP_LOGI(TAG, "announcing %s.%s.%s.local fw=%s prof=%s cr=%s err=%s", host, SERVICE, PROTO,
             sebastian_fw_version(), prof ? prof : "", cr ? cr : "", boot_err);
}
