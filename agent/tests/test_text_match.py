...
def test_norm_lowercases_strips_accents_and_punctuation() -> None:
    assert norm("¡Hóla, Múndo!").split() == ["hola", "mundo"]
    assert norm("Árbol ñandú").split() == ["arbol", "nandu"]


def test_shingles_uses_bare_words_below_three() -> None:
    assert shingles("hola mundo") == {"hola", "mundo"}
    assert shingles("uno") == {"uno"}
    assert shingles("") == set()


def test_shingles_three_word_windows() -> None:
    assert shingles("a b c d") == {"a b c", "b c d"}


def test_containment_full_partial_and_empty() -> None:
